...
def get_earnings_data(date_str):
    ...
    headers = {
        ...
        'Referer': f'https://www.earningswhispers.com/calendar/{date_str}',
...
# ... (Other helper functions like parse_time, parse_impact, etc. remain unchanged)
...
def main():
    ...
    if st.sidebar.button("🔄 Fetch Live Data", type="primary"):
        with st.spinner("Fetching data..."):
            try:
                # Assuming ffscraper.py and this script are in the same directory
                subprocess.run([sys.executable, "ffscraper.py"], check=True, capture_output=True, text=True)
                # You might want to add update_db_from_csv logic here if needed
    ...
    if df.empty:
        st.warning("👋 No economic data found. Click **Fetch Live Data** in the sidebar to load events.")
        return
